...
**Statut : Lots 0 à 11 (backend) et les 13 lots frontend (client, SPLIT, wallet, profil, kiosque, les 25 écrans admin, écrans secondaires, changement d'e-mail, rapport de chiffre d'affaires, création de réservation admin, planning en grille, revue de migration Doinsport) terminés — 259 tests verts, CI en place.** Le modèle de données, l'import Doinsport (clients + réservations), l'anti-collision Dual Run (CDC §10.3), l'écran de revue des conflits de migration, le tableau de bord des imports (`LegacySyncRun`), le scheduler récurrent (`LegacySyncScheduler`, CDC §15.3 : sync fréquente 60 s + réconciliation 300 s) et le flux de rédemption de l'invitation de migration (CDC §7.3, page `/migrate`) sont construits et vérifiés en conditions réelles. Reste : la migration par cohortes et le cutover (voir [`PLAN_ACTION.md`](PLAN_ACTION.md) pour le détail lot par lot et [`docs/migration.md`](docs/migration.md) pour la suite). Un compte Stripe **test** réel est désormais connecté — paiement par carte vérifié en direct (checkout, packs de crédits, carte enregistrée) ; reste le passage en mode live et Bancontact/iDEAL/Apple Pay/Terminal.
