...
Traçabilité CDC Annexe B (44 items) — mise à jour au fil des lots plutôt qu'en fin de projet, pour que "on est prêt pour le pilote" repose sur une preuve item par item et non sur une impression. Complète `docs/migration.md` (Phase 0 → Phase 1) : la sortie de Phase 0 exige "suite de tests verte, staging fonctionnel" — la suite de tests l'est (259 tests, 42 fichiers) ; **le staging ne l'est pas encore** (aucune infrastructure de déploiement au-delà de `docker-compose.yml` pour Postgres local — voir "Ce qui bloque réellement" en bas de page).
...
1. **Un environnement de staging réel.** `docker-compose.yml` ne provisionne qu'une base Postgres locale — aucune configuration de déploiement (hébergement API/frontend, domaine, certificats) n'existe encore. Sans ça, la sortie de Phase 0 ("staging fonctionnel") n'est pas atteinte, et la Phase 1 ne peut pas commencer ailleurs qu'en local.
