...
### Addendum 5 (2026-08-18) — CA planning étendu aux réservations Doinsport

Question utilisateur : le CA jour/semaine/mois (Addendum 4) ne comptait que les réservations V2, faute de prix stocké pour les occupations Doinsport — pourtant le prix par participant (`participants[].price`) est déjà lu à l'import pour calculer `fullyPaid` (Addendum 2), simplement jamais persisté.

Deux nouveaux champs sur `LegacyBooking` : `priceDueCents` (somme des prix des participants non annulés) et `priceReceivedCents` (somme des paiements `succeeded`) — `fullyPaid` reste dérivé des deux à l'import, inchangé en usage. Un champ `priceCents` sur `LegacyBookingParticipant` (prix dû par ce participant précisément, fiable — porté directement par l'objet participant Doinsport, contrairement au statut de paiement qui reste uniquement agrégé au niveau réservation, cf. Addendum 2 : `payments[].participantId` ne correspond toujours à aucun identifiant exposé dans `participants[]`).

`booking-participants.ts` refactorisé : `computeFullyPaid` s'appuie maintenant sur deux fonctions exportées séparément, `computeDueCents`/`computeReceivedCents`, réutilisées telles quelles pour peupler les nouveaux champs sans dupliquer la logique de somme.

Planning : le CA inclut désormais `priceDueCents` des occupations Doinsport de la période (même fenêtre combinée semaine/mois que le fetch V2 existant, nouvel appel `/admin/legacy-bookings` en parallèle). Basé sur le prix dû (vente), pas le montant encaissé — même convention que le CA V2, qui ne filtre pas non plus sur le statut de paiement (une réservation confirmée compte pour son prix, encaissée ou non).

Migration `20260818154239_legacy_booking_prices`, appliquée aux deux bases (dev + test). Réimport ciblé (`npm run import:legacy -- --target=bookings --from=2026-08-17 --to=2026-08-20`) pour peupler les prix des réservations déjà synchronisées sur la fenêtre de vérification — même limitation historique que le compteur de réservations (Addendum 2) : les ~4284 réservations passées déjà en base avant ce lot n'ont pas de prix tant qu'elles ne sont pas resynchronisées, pas de rattrapage complet fait ici.

Vérifié en direct : CA jour passé de 0,00 € à 180,00 € après réimport (auparavant nul faute d'inclure Doinsport), aucune erreur console, build et lint propres. 271 tests backend verts (43 fichiers, dont 1 nouveau dans `booking-participants.test.ts` et assertions étendues dans `legacy-sync-scheduler.test.ts`/`bookings-admin.service.test.ts`).

### Addendum 6 (2026-08-18) — CA ventilé par canal (Stripe / wallet / Doinsport)

Demande utilisateur : distinguer, dans le CA du planning, ce qui est passé par Stripe, par le wallet, ou par Doinsport.

Nouvelle route `GET /admin/revenue-by-channel?from=&to=`, calculée entièrement côté serveur plutôt que resommée côté client à partir de `/admin/bookings` : la ventilation Stripe/wallet nécessite de croiser `Payment` et `WalletTransaction`, deux tables qui n'étaient pas incluses dans la réponse `/admin/bookings` existante (utilisée ailleurs, notamment la liste des réservations — l'étendre aurait alourdi cette route pour tous ses consommateurs). `BookingsRepository.sumRevenueByChannelInRange` récupère d'abord les ids des réservations `CONFIRMED`/`COMPLETED` de la période, puis agrège séparément `Payment` (`status=SUCCEEDED`, `purpose` réservation — pas achat de pack crédits) et `WalletTransaction` (`type=DEBIT_BOOKING`, montant signé négatif en base, valeur absolue prise ici) sur ces ids : `WalletTransaction` n'a pas de relation Prisma vers `Booking` (simple `bookingId`), un filtre imbriqué à une requête n'était donc pas possible. `BookingsAdminService.revenueByChannel` complète avec le total Doinsport (`priceDueCents`, déjà utilisé pour le CA total depuis l'addendum 5).

Côté planning, remplace l'ancien calcul client (`periodBookings`/`periodLegacyOccupations` resommés localement) par trois appels à cette route (jour/semaine/mois) — plus cher en requêtes réseau (3 vs 2) mais radicalement plus simple que de répliquer une jointure Payment/WalletTransaction côté client, et le total reste affiché en plus de la ventilation (`formatRevenueBreakdown`), donc pas de perte d'information par rapport à avant.

Vérifié : 272 tests backend verts (nouveau test `bookings-admin.service.test.ts` créant une réservation payée Stripe, une payée wallet et une occupation Doinsport dans la même fenêtre, vérifiant les trois totaux exacts), build et lint frontend propres.
