...
6. **Provisionner un environnement de staging réel** — `docker-compose.yml` ne fournit qu'une base Postgres locale, aucune infrastructure de déploiement (hébergement API/frontend, domaine, certificats) n'existe encore. Bloque formellement la sortie de la Phase 0 ("staging fonctionnel", `docs/migration.md`) autant que Stripe bloque les items de paiement.
