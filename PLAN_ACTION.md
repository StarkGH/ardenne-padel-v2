...
**Mise à jour — artefacts de staging posés (voir ADR-0038).** `apps/api/Dockerfile`, `apps/web/Dockerfile` (build standalone Next.js), `docker-compose.staging.yml` (db + api + web + reverse proxy Caddy avec TLS automatique), runbook complet (`docs/deployment.md`). **Un bug de production bloquant a été trouvé et corrigé en le préparant** : le build compilé (`npm run build`) produisait un `dist/server.js` qui ne démarrait jamais (`node dist/server.js` plantait immédiatement, résolution de module cassée entre les packages partagés du monorepo — `main`/`types` pointaient vers du TypeScript brut au lieu du JS compilé) — jamais détecté en 11 lots backend + 13 lots frontend car aucun n'avait exécuté ce chemin de bout en bout (seuls `tsx`/`vitest`, qui transpilent à la volée, avaient jamais tourné). Corrigé via un `exports` conditionnel par package (`development` → source live pour `tsx`/`vitest`, `default` → `dist/` compilé pour la production) — vérifié par un build complet depuis zéro suivi d'une vraie requête HTTP contre le serveur compilé (`401` sur `/api/v1/auth/me`), et par la suite de tests inchangée (259 tests verts). `.gitignore` corrigé au passage (`.env.staging` n'était pas exclu). Restant : provisionner un vrai serveur et un domaine (décision opérationnelle du club, hors périmètre code), confirmer les Dockerfiles avec un vrai `docker build` (démon Docker indisponible dans l'environnement de développement utilisé pour ce lot — accès `sudo` non disponible), script de provisionnement du premier compte admin, CI/CD de déploiement.
...
1. ~~Ouvrir un compte Stripe réel pour Ardenne Padel~~ — **compte test connecté et vérifié en direct** (ADR-0037-stripe, 2026-08-17 : Stripe Elements carte sur les 4 parcours de paiement, checkout/packs de crédits/carte enregistrée réellement testés). Restant : passage en mode **live**, moyens de paiement locaux (Bancontact/iDEAL) et Apple/Google Pay (V-012), Terminal (V-014).
...
6. ~~Provisionner l'environnement de staging~~ — **artefacts posés** (voir ADR-0038, `docs/deployment.md`) : Dockerfiles API/web, `docker-compose.staging.yml`, reverse proxy Caddy (TLS automatique). **Un bug bloquant de production trouvé et corrigé au passage** : le build compilé ne démarrait pas du tout (`node dist/server.js` plantait, résolution de module cassée pour les packages partagés), jamais détecté car ce chemin n'avait jamais été exécuté de bout en bout avant ce lot. Reste : provisionner un vrai serveur + domaine (décision opérationnelle du club) et confirmer les Dockerfiles avec un vrai `docker build` (démon Docker indisponible dans l'environnement de développement utilisé pour ce lot).
7. **Suivi Annexe B créé** ([`docs/annexe-b-checklist.md`](docs/annexe-b-checklist.md), 2026-08-17) : 44 items du CDC tracés item par item avec preuve — 24 pleinement vérifiés en conditions réelles, 9 codés/testés mais jamais exercés avec de vraies données externes (majoritairement bloqués par Stripe), 6 dette opérationnelle documentée (terrains/tarifs réels, sauvegardes automatiques, monitoring externe, audit de sécurité externe), 5 non faits ou hors de portée sans prérequis externe (dont E2E Playwright, jamais mis en place — la vérification de ce projet s'est toujours faite manuellement en navigateur à chaque lot).
