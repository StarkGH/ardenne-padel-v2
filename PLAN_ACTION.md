...
**Les 11 lots backend et les 12 lots frontend décrits ci-dessus sont tous committés et testés** (232 tests backend verts en CI, voir chaque section pour le détail de ce qui reste par lot). Les 25 écrans admin, les écrans client/kiosque secondaires, le changement d'e-mail et le rapport de chiffre d'affaires sont désormais construits. Ce qui bloque encore un vrai pilote : un compte Stripe réel pour Ardenne Padel (aucun parcours de paiement n'a été validé en conditions réelles, tout se dégrade proprement en 503, à l'exception des paiements 100 % wallet — réellement aboutis en direct), les validations juridiques/comptables V-018 à V-024 (frais SPLIT, TVA crédits — hors code), et la synchro Doinsport récurrente (le modèle de données et le script d'import initial sont posés, Lot 11/ADR-0031-0032 — vérifiés en conditions réelles, 1090 clients + réservations importées — mais aucun scheduler ne le rejoue automatiquement, et l'anti-collision Dual Run n'est pas encore branchée sur les données importées).
