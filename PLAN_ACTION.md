...
6. ~~Provisionner l'environnement de staging~~ — **artefacts posés** (voir ADR-0037, `docs/deployment.md`) : Dockerfiles API/web, `docker-compose.staging.yml`, reverse proxy Caddy (TLS automatique). **Un bug bloquant de production trouvé et corrigé au passage** : le build compilé ne démarrait pas du tout (`node dist/server.js` plantait, résolution de module cassée pour les packages partagés), jamais détecté car ce chemin n'avait jamais été exécuté de bout en bout avant ce lot. Reste : provisionner un vrai serveur + domaine (décision opérationnelle du club) et confirmer les Dockerfiles avec un vrai `docker build` (démon Docker indisponible dans l'environnement de développement utilisé pour ce lot).
